  ...
  let findPath = function (){
    ...
    if (!ship_name || !mod_name) {
      ...
      $.get(link).then(function(mod_code){
          ...
          for (let level in internals.ships) {
            ...
            if (current_ships.indexOf(code) != -1){
              ...
              if (Array.isArray(custom_next)) {
                ...
                return uAr(cnext).forEach((ship, i) => getNextShipCodes(ship, path.concat([ship, i])));
              }
              else {
                ...
                return next_ships.slice(alpha, alpha + 2).forEach((ship, i) => getNextShipCodes(ship, path.concat([ship, i])));
              }
            }
          }
        ...
        for (let i of Object.values(internals.ships)[0]) getNextShipCodes(i, []);
