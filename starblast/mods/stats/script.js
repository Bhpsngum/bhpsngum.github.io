(function(){
  ...
  }, img_size = 360, padding_ratio = 1/15, margin_ratio = 1/30, adjust = 5, full_ratio = 1+2*padding_ratio+2*margin_ratio, adjustwidth = function(){
    let g = $(window).width(), x = Math.round(g/(img_size*full_ratio)), t = Math.max(g/(x||1)/full_ratio - adjust,0), m = Math.trunc(t*padding_ratio);
    console.log(x,t);
    $(".modStatBox").css({width: Math.trunc(t*(1+2*padding_ratio))+"px",padding: m+"px", margin: Math.trunc(t*margin_ratio)+"px","border-radius":m+"px"});
  }
