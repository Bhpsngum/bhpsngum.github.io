  ...
  modStatBox = function(stat, count, index, time) {
    ...
    if (parent.length == 0) $('#modstats').append(`<div index = "${index}" class="modStatBox" id='${stat.mod_id}'>${img}<div id="stat-${stat.mod_id}">${statinfo}</div></div>`);
      ...
      if (u.length > 0 && stat.active && player_count[stat.mod_id]) {
        let c = "<td>:</td>", evt = "<tr id='total_players-"+stat.mod_id+"'>", totalplayers = "<td><b>Current players</b</td>"+c+"<td class='numbers'>"+player_count[stat.mod_id]+"</td>", getStat = function(t){return u.filter(i => player_count_region[stat.mod_id][i]).map(i => `<tr ${$("#players-"+stat.mod_id).attr("hide") == "true"||t?"hidden":""}><td>${i}</td>${c}<td class='numbers'>${player_count_region[stat.mod_id][i]}</td></tr>`).join("")};
