(function(){
  ...
  created_time = {
    ...
    statinfo = `<h3 style="text-align:center"><a style="text-decoration:none" href="https://starblastio.gamepedia.com/${stat.title.replace(/\s/g,"_")}" target="_blank">${stat.title}</a> <sup>${stat.version}</sup></h3>${stat.new&&stat.active?'<b style="color:yellow;float:right">NEW!</b>':""}${!stat.active?'<b style="color:red;float:right">Removed</b>':""}`;
